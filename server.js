...
app.post("/api/event",(req,res)=>{
  ...
  const now=Date.now();
  db.run(`INSERT INTO devices VALUES (?,?,?)
  ...
  if(event==="SYNC" && uptime_ms){
    const fakeTime=new Date(Date.now()-uptime_ms).toISOString();
  ...
  if(event==="STATE_SYNC"){
    ...
    db.run(`INSERT INTO events VALUES (NULL,?,?,?,?,CURRENT_TIMESTAMP)`,
