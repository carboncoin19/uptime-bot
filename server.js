...
function formatTime(ms){
  return new Date(ms+TZ_OFFSET_MS).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:true});
}
...
function broadcast(t){
  db.all(`SELECT chat_id FROM chats`,(_,r)=>r?.forEach(x=>tg(x.chat_id,t)));
}

/* ---------- SLA ---------- */
function sla(ms){return Math.min(100,(ms/86400000)*100);}
...
let lastId=0;
setInterval(async()=>{
  const r=await fetch(`https://api.telegram.org/bot${TG_BOT_TOKEN}/getUpdates?offset=${lastId+1}`)
